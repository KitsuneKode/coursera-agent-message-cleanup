...
Reads from your clipboard, removes all Coursera boilerplate, writes the cleaned text back to your clipboard. **That's it.**
...
- **Python 3.10+**
- **pyperclip** (installed automatically via `uv sync` or `pip install pyperclip`)
...
**Clipboard does not update on Linux** — Install the appropriate clipboard backend for your display server (see table above). The script prints a warning to stderr with install commands if clipboard access fails.
